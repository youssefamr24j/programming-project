...
void Output::DrawRhombus(Point P, GfxInfo GfxInfo, bool selected) const
{
	int arrayx[4] = { P.x - 100,P.x,P.x + 100,P.x };
	int arrayy[4] = { P.y,P.y + 100,P.y,P.y - 100 };
	color DrawingClr;
	if (selected)
		DrawingClr = UI.HighlightColor; //Figure should be drawn highlighted
	else
	...
	pWind->DrawPolygon(arrayx, arrayy, 4, style);

}
void Output::DrawEll(Point P1, Point P2, GfxInfo GfxInfo, bool selected) const
{
	color DrawingClr;
	if (selected)
		DrawingClr = UI.HighlightColor; //Figure should be drawn highlighted
